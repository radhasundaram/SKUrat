...
test_prep<-function(df,clusterinfo,dummycolnames,flavours){
  ...
  #df$Product.Type<-ifelse(!(df$Product.Type%in%c("Milk Choc")),"Others",df$Product.Type)
  ...
  df %<>% mutate(Packsize_grouped=ifelse(Packsize<160,"<160",
                                         ifelse(Packsize>=160&Packsize<250,"[160,250)",
                                                ifelse(Packsize>=250&Packsize<340,"[250,340)",">=340"))))
...
clean_skusim<-function(sim_scores){
  # get similar skus to the discontinued sku
...
n_distinct(all_discontinued_skus) #16
n_distinct(normal_skus) #23
...
n_distinct(data_segment_temp$APN) # 29
nrow(data_segment_temp)# 924
...
n_distinct(data_segment_mars$APN) # 29
nrow(data_segment_mars) # 842
...
#data_segment_mars$Product.Type<-ifelse(!(data_segment_mars$Product.Type%in%c("Milk Choc")),"Others",data_segment_mars$Product.Type)
...
data_segment_mars$Packsize_grouped <-as.character(cut2(data_segment_mars$Packsize, g =4))
data_segment_mars$Packsize_grouped<-ifelse(data_segment_mars$Packsize_grouped=="[140,160)","<160",
                                           ifelse(data_segment_mars$Packsize_grouped=="[340,380]",">=340",data_segment_mars$Packsize_grouped))
...
dummycolnames=c("cluster","Promo.Group","Sub.Segment","Product.Type","Packsize_grouped","Flavour")
...
for(i in seq_along(all_discontinued_skus)){
  ...
  sku_temp=all_discontinued_skus[i]
  print(sku_temp)
  disc_date_temp<-discontinued_skus_mod %>% filter(APN==sku_temp) %>% .$ActualDiscDate
  disc_date<-disc_date_temp
  month(disc_date) <- month(disc_date) + 3
  fil_temp<-ModelData %>% 
    filter(APN == sku_temp,Date<=disc_date)%>% 
    mutate(trans_flag=ifelse(Date>disc_date_temp,1,0))
  
  data_till_discontinue = rbind(data_till_discontinue,fil_temp)
  rest_of_data[,paste0("temp_flag",i)] = ifelse(rest_of_data$Date>disc_date_temp&rest_of_data$Date<=disc_date,1,0)  
}

rest_of_data$trans_flag<-eval(parse(text=paste0("rest_of_data$temp_flag",seq_along(all_discontinued_skus),collapse = "|")))
rest_of_data$trans_flag<-ifelse(rest_of_data$trans_flag,1,0)
rest_of_data<-rest_of_data %>% select(-starts_with("temp_flag"))
...
test_data_prepared$`Sales Lost`<-test_data_prepared$Prediction_oldassort_zeroflag
...
# sales lost changed to just the zero flag old assortment estimate
# disc period limited to 3 months after flagging disc
# Indicated the transition period for both normal and disc skus
...
write.xlsx(list,paste0("Output/",Segment_fil,"_modelresults_(General)_re2.xlsx"))
