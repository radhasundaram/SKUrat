...
test_prep<-function(df,clusterinfo,dummycolnames,flavours){
  ...
  # df=test_data
  # test_skus<-unique(df$APN)
  # clusterinfo=df_clust
  ...
  #flavours<-setdiff(unique(df$Flavour),c("Other Flavour"))
  
  #prop.table(table(df$Packsize)) %>% View(.)
  #prop.table(table(df$Packsize_grouped)) %>% View(.)
  #unique(df$Packsize_grouped)
  df$Packsize=sapply(df$Packsize,function(x)as.numeric(sub(x,pattern = "g",replacement = "")))
  df %<>% mutate(Packsize_grouped=ifelse(Packsize<46,"<46",ifelse(Packsize>=46&Packsize<60,"[46,60)",">=60")))
...
data_segment = data %>% 
  ...
  arrange(APN,Date) 
n_distinct(data_segment$APN) # 38
nrow(data_segment) # 990
...
data_segment_temp=data_segment%>% 
  ...
  slice(.,3:n()) %>% 
  ungroup()
n_distinct(data_segment_temp$APN) #38
nrow(data_segment_temp) # 914
...
n_distinct(data_segment_temp$APN) #30
nrow(data_segment_temp)# 859
...
n_distinct(all_discontinued_skus) #13
n_distinct(normal_skus) #13
...
n_distinct(data_segment_mars$APN) # 16
nrow(data_segment_mars) # 486
...
data_segment_mars$Packsize_grouped <-as.character(cut2(data_segment_mars$Packsize, g =3))
data_segment_mars$Packsize_grouped<-ifelse(data_segment_mars$Packsize_grouped=="[33,46)","<46",ifelse(data_segment_mars$Packsize_grouped=="[60,80]",">=60",data_segment_mars$Packsize_grouped))
