...
test_prep<-function(df,clusterinfo,dummycolnames,flavours){
  ...
  df %<>% mutate(Packsize_grouped=ifelse(Packsize<180,"<180",ifelse(Packsize>=180&Packsize<307,"[180,307)",">=307")))
...
data_segment = data %>% 
  ...
  arrange(APN,Date) 
n_distinct(data_segment$APN) # 17
nrow(data_segment) # 531
...
data_segment_temp=data_segment%>% 
  ...
  slice(.,3:n()) %>% 
  ungroup()
n_distinct(data_segment_temp$APN) #17
nrow(data_segment_temp) # 497
...
n_distinct(all_discontinued_skus) #2
n_distinct(normal_skus) #11
...
n_distinct(data_segment_mars$APN) # 13
nrow(data_segment_mars) # 402
...
data_segment_mars$Packsize_grouped<-ifelse(data_segment_mars$Packsize_grouped=="[144,180)","<180",ifelse(data_segment_mars$Packsize_grouped=="[307,360]",">=307",data_segment_mars$Packsize_grouped))
...
n_distinct(data_segment_test$APN)#17
